  ...
  db.query(
    ...
    (err, results) => {
      if (err || results.length === 0)
        return res.status(500).json({ msg: "Erro verificação owner" });
...
app.get("/api/public/user/:id", (req, res) => {
  const sql =
    "SELECT id, name, avatar, bio, social_links, likes_public, plans_public, recipes_public, created_at FROM users WHERE id = ?";
...
app.put("/api/user/profile", verifyToken, (req, res) => {
  ...
  if (likes_public !== undefined) {
    isPublic =
      likes_public === true || likes_public === 1 || likes_public === "1"
        ? 1
        : 0;
  }
 const sql =
  "UPDATE users SET bio = ?, avatar = ?, social_links = ?, likes_public = ?, plans_public = ?, recipes_public = ? WHERE id = ?";
    db.query(sql, [
  bio,
  avatar,
...
app.get("/api/public/user/:id/likes", verifyToken, (req, res) => {
  ...
  db.query(
    "SELECT likes_public FROM users WHERE id = ?",
    [targetUserId],
    (err, userRes) => {
...
// --- PLANOS PÚBLICOS ---
app.get("/api/public/user/:id/plans", verifyToken, (req, res) => {
  const targetId = req.params.id;
  db.query("SELECT plans_public FROM users WHERE id = ?", [targetId], (err, r) => {
    if (!r || r.length === 0) return res.sendStatus(404);
    if (!r[0].plans_public && req.userId != targetId) return res.sendStatus(403);

    db.query("SELECT data FROM saved_plans WHERE user_id = ?", [targetId], (err, plans) => {
      res.json(plans ? plans.map(p => p.data) : []);
    });
  });
});

// --- RECEITAS PÚBLICAS ---
app.get("/api/public/user/:id/recipes", verifyToken, (req, res) => {
  const targetId = req.params.id;
  db.query("SELECT recipes_public FROM users WHERE id = ?", [targetId], (err, r) => {
    if (!r || r.length === 0) return res.sendStatus(404);
    if (!r[0].recipes_public && req.userId != targetId) return res.sendStatus(403);

    db.query("SELECT data FROM recipes WHERE user_id = ?", [targetId], (err, recipes) => {
      res.json(recipes ? recipes.map(r => r.data) : []);
    });
  });
});
...
app.post("/api/community/post", verifyToken, (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      ...
      if (results[0].can_post === 0)
        return res.status(403).json({ msg: "Proibido postar." });
      const sql =
        "INSERT INTO community_posts (user_id, title, description, youtube_link, content_json) VALUES (?, ?, ?, ?, ?)";
      db.query(
        sql,
        [
          req.userId,
...
app.post("/api/community/report/comment", verifyToken, (req, res) => {
  ...
  const sql =
    "INSERT INTO community_comment_reports (comment_id, reporter_id, reason) VALUES (?, ?, ?)";
...
app.post("/api/community/vote", verifyToken, (req, res) => {
  ...
  db.query(
    "SELECT * FROM community_votes WHERE post_id = ? AND user_id = ?",
    [post_id, req.userId],
    (err, results) => {
      if (err) return res.status(500).json({ msg: "Erro SQL" });
      if (results.length > 0) {
        ...
        if (currentVote === vote_type) {
          db.query(
            "DELETE FROM community_votes WHERE id = ?",
            [results[0].id],
            (err) => res.json({ msg: "Removido" })
          );
        } else {
          db.query(
            "UPDATE community_votes SET vote_type = ? WHERE id = ?",
            [vote_type, results[0].id],
            (err) => res.json({ msg: "Atualizado" })
          );
        }
      } else {
        db.query(
          "INSERT INTO community_votes (post_id, user_id, vote_type) VALUES (?, ?, ?)",
          [post_id, req.userId, vote_type],
          (err) => res.json({ msg: "Computado" })
        );
      }
    }
  );
...
app.get("/api/library", verifyToken, (req, res) => {
  db.query(
    "SELECT data FROM recipes WHERE user_id = ?",
    [req.userId],
    (err, r) => res.json(r ? r.map((x) => x.data) : [])
  );
});
app.post("/api/library", verifyToken, (req, res) => {
  const r = req.body;
  db.query(
    "DELETE FROM recipes WHERE user_id=? AND front_id=?",
    [req.userId, r.id],
    () => {
      db.query(
        "INSERT INTO recipes (user_id,front_id,data) VALUES (?,?,?)",
        [req.userId, r.id, JSON.stringify(r)],
        (e) => res.json({ msg: "Salvo" })
      );
    }
  );
});
...
app.get("/api/presets", verifyToken, (req, res) => {
  db.query(
    "SELECT data FROM saved_plans WHERE user_id = ? ORDER BY created_at DESC",
    [req.userId],
    (err, r) => res.json(r ? r.map((x) => x.data) : [])
  );
});
app.post("/api/presets", verifyToken, (req, res) => {
  const p = req.body;
  db.query(
    "INSERT INTO saved_plans (user_id,front_id,name,data) VALUES (?,?,?,?)",
    [req.userId, p.id, p.name, JSON.stringify(p)],
    (e) => res.json({ msg: "Salvo" })
  );
});
app.put("/api/presets/:id", verifyToken, (req, res) => {
  const { name } = req.body;
  db.query(
    "SELECT data FROM saved_plans WHERE user_id = ? AND front_id = ?",
    [req.userId, req.params.id],
    (err, results) => {
      if (err || results.length === 0)
        return res.status(500).json({ error: "Erro/Não encontrado" });
      let planData = results[0].data;
      planData.name = name;
      db.query(
        "UPDATE saved_plans SET name = ?, data = ? WHERE user_id = ? AND front_id = ?",
        [name, JSON.stringify(planData), req.userId, req.params.id],
        (err) => {
          if (err) return res.status(500).json({ msg: "Erro SQL" });
...
app.delete("/api/presets/:id", verifyToken, (req, res) => {
  db.query(
    "DELETE FROM saved_plans WHERE user_id = ? AND front_id = ?",
    [req.userId, req.params.id],
    (err) => {
      if (err) return res.status(500).json({ msg: "Erro SQL" });
...
app.get("/api/planner", verifyToken, (req, res) => {
  db.query(
    "SELECT planner_data, themes_data FROM app_state WHERE user_id = ?",
    [req.userId],
    (err, results) => {
      if (err) return res.status(500).json({ msg: "Erro SQL" });
...
app.post("/api/planner", verifyToken, (req, res) => {
  ...
  db.query(
    sql,
    [req.userId, JSON.stringify(planner), JSON.stringify(themes)],
    (err) => {
      if (err) return res.status(500).json({ msg: "Erro SQL" });
...
app.get("/api/owner/stats", verifyToken, verifyOwner, (req, res) => {
  ...
  const qPosts = new Promise((resolve, reject) =>
    db.query("SELECT COUNT(*) as total FROM community_posts", (err, res) =>
      err ? reject(err) : resolve(res[0].total)
    )
  );
  const qReports = new Promise((resolve, reject) =>
    db.query(
      'SELECT COUNT(*) as total FROM community_reports WHERE status = "pending"',
      (err, res) => (err ? reject(err) : resolve(res[0].total))
    )
  );
  Promise.all([qUsers, qPosts, qReports])
    .then(([users, posts, reports]) => res.json({ users, posts, reports }))
    .catch((err) => res.status(500).json({ msg: "Erro" }));
});
