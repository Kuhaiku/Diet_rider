...
// --- VARIÁVEIS GLOBAIS ---
let allPosts = []; // Armazena posts do feed
let myLibrary = []; // Armazena receitas do usuário (para anexar)
let myPresets = []; // Armazena planos do usuário (para anexar)
let selectedAttachments = new Set(); // Controle de seleção no modal de anexo

// --- INICIALIZAÇÃO ---
// Só roda o onload se estiver na página da comunidade ou se o elemento existir
if (
  ...
  window.onload = async function () {
    ...
    injectPreviewModal(); // Injeta o HTML do modal de detalhes
    setupSidebarLinks(); // Configura links do perfil na sidebar

    // Carregamento paralelo
    await Promise.all([
      loadPosts(),
      fetchUserLibrary(), // Importante: Carrega seus dados para poder anexar depois
    ]);
...
async function loadPosts() {
  if (!document.getElementById("community-feed")) return; // Proteção para página de perfil

  try {
    const res = await fetch(`${API_BASE}/community/posts`, { headers });
    if (!res.ok) throw new Error("Erro ao carregar feed");
    const posts = await res.json();
    allPosts = posts; // Salva globalmente
...
// Função Flexível: Renderiza feed no alvo especificado (Comunidade ou Perfil)
function renderFeed(posts, targetId = "community-feed") {
  ...
  if (!posts || posts.length === 0) {
    feed.innerHTML = `<div class="text-center py-10 text-slate-400">Nenhuma postagem ainda.</div>`;
    return;
  }
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden fade-in relative group mb-6 scroll-mt-24" id="post-${
          p.id
            ...
            <div class="p-4 flex justify-between items-start">
                <div class="flex items-center gap-3 cursor-pointer hover:opacity-80 transition-opacity" onclick="window.location.href='/src/perfil.html?id=${
                  p.user_id
                }'">
                    ${avatarHtml}
                    <div>
      ...
      p.created_at
            ...
            <div class="px-4 py-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <button onclick="vote(${
                      p.id
                    }, 'like')" class="flex items-center gap-2 px-3 py-1.5 rounded-full hover:bg-green-100 text-slate-500 hover:text-green-600 transition-colors ${
      userVote === "like"
        ...
        : ""
                    ...
                    <button onclick="vote(${
                      p.id
                    }, 'dislike')" class="flex items-center gap-2 px-3 py-1.5 rounded-full hover:bg-red-100 text-slate-500 hover:text-red-600 transition-colors ${
      userVote === "dislike"
...
// --- SISTEMA DE ANEXO (RESTAUTADO) ---

// Busca dados do usuário (Receitas e Planos) para preencher o modal de anexo
async function fetchUserLibrary() {
  try {
    const resLib = await fetch(`${API_BASE}/library`, { headers });
    if (resLib.ok) {
      const data = await resLib.json();
      myLibrary = Array.isArray(data) ? data : [];
    }

    const resPre = await fetch(`${API_BASE}/presets`, { headers });
    if (resPre.ok) {
      const data = await resPre.json();
      myPresets = Array.isArray(data) ? data : [];
    }
  } catch (err) {
    console.error("Falha ao carregar biblioteca do usuário para anexos", err);
  }
}
...
function openAttachModal() {
  ...
  loadAttachList("recipe"); // Carrega receitas por padrão
}
...
function loadAttachList(type) {
  ...
  if (type === "recipe") {
    ...
    if (!myLibrary || myLibrary.length === 0) {
      list.innerHTML =
        '<div class="p-4 text-center text-sm text-slate-400">Nenhuma receita encontrada.</div>';
      return;
    }
    ...
    myLibrary.forEach((r) => {
      ...
      el.className =
        "p-3 hover:bg-slate-50 border-b border-slate-50 cursor-pointer flex items-center gap-3 select-none rounded-lg";
      el.innerHTML = `
    ...
    if (!myPresets || myPresets.length === 0) {
      list.innerHTML =
        '<div class="p-4 text-center text-sm text-slate-400">Nenhum plano encontrado.</div>';
      return;
    }
    ...
    myPresets.forEach((p) => {
      const el = document.createElement("div");
      el.className =
        "p-3 hover:bg-slate-50 border-b border-slate-50 cursor-pointer flex items-center gap-3 rounded-lg";
      el.innerHTML = `<i class="fa-solid fa-calendar-days text-indigo-400"></i><span class="text-sm font-medium text-slate-700">${p.name}</span>`;
...
function confirmRecipePackage() {
  if (selectedAttachments.size === 0)
    return notify("Selecione pelo menos uma receita.", "error");
  ...
  const label =
    packageList.length === 1
      ? `Receita: ${packageList[0].name}`
      : `Pacote: ${packageList.length} Receitas`;
...
function openPreviewModalGeneric(content, title, isImportMode) {
  ...
  if (!container) {
    // Se chamado fora da comunidade/perfil que já injetou, tenta injetar
    injectPreviewModal();
