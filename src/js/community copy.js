...
async function loadPosts(query = "") {
    ...
    try {
        ...
        if(el) el.innerHTML = `<div class="text-center py-10 text-red-400 font-bold">Erro ao carregar posts.</div>`;
    }
}
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden fade-in relative group mb-6 scroll-mt-24" id="post-${p.id}">
            ...
            <div class="p-4 flex justify-between items-start">
                <div class="flex items-center gap-3 cursor-pointer hover:opacity-80 transition-opacity" onclick="window.location.href='perfil.html?id=${p.user_id}'">
                    ${avatarHtml}
                    <div>
                        <h3 class="font-bold text-slate-800 leading-tight hover:text-indigo-600 transition-colors">${p.title}</h3>
            ...
            <div class="px-4 py-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    <button onclick="sharePost(${p.id}, ${p.user_id})" class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-blue-100 text-slate-400 hover:text-blue-600 transition-colors" title="Link Direto">
                        <i class="fa-solid fa-link"></i>
                    </button>
                    <button onclick="prepareImport(${p.id})" class="px-4 py-1.5 bg-slate-800 text-white text-xs font-bold rounded-full hover:bg-slate-900 shadow-md flex items-center gap-2 transition-transform active:scale-95">
...
async function vote(postId, type) { 
    ...
    try { 
        ...
        if (res.ok) { 
            ...
            if (typeof loadProfileLikes === 'function' && document.getElementById('tab-likes') && document.getElementById('tab-likes').classList.contains('border-indigo-600')) {
                loadProfileLikes(); 
            } 
            // Verifica se está na aba "Posts" do perfil
            else if (typeof loadProfilePosts === 'function' && document.getElementById('tab-posts') && document.getElementById('tab-posts').classList.contains('border-indigo-600')) {
                loadProfilePosts();
            } 
    ...
    items.forEach(i => {
        ...
        div.innerHTML = `
            ...
            <label for="${chkId}" class="flex-1 cursor-pointer font-medium text-sm text-slate-700 flex items-center gap-2">
                ${type === 'recipe' ? `<span class="w-6 h-6 rounded bg-slate-100 flex items-center justify-center text-xs text-slate-500"><i class="fa-solid ${i.icon || 'fa-utensils'}"></i></span>` : '<i class="fa-solid fa-calendar-days text-indigo-400"></i>'}
                ${i.name}
            </label>`;
        div.onclick = () => { document.getElementById(chkId).click(); };
...
function confirmRecipePackage() {
    if (selectedAttachments.size === 0) return notify("Selecione pelo menos um item.", "error");
    ...
    const label = packageList.length === 1 ? `Anexo: ${packageList[0].name}` : `Pacote: ${packageList.length} Itens`;
...
function openPreviewModalGeneric(content, title, isImportMode) {
    ...
    if(titleEl) titleEl.innerText = isImportMode ? "Confirmar Importação" : title;
    ...
    if(actionBtn) {
        if(isImportMode) {
            ...
            actionBtn.className = "px-6 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-bold rounded-lg shadow-lg";
        } else {
    ...
    const html = `
        <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm mb-4">
            <div class="flex items-center gap-3 mb-4 border-b border-slate-100 pb-3">
                <div class="w-12 h-12 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center text-xl"><i class="fa-solid ${recipe.icon || "fa-utensils"}"></i></div>
                <div><h4 class="font-bold text-slate-800 text-lg">${recipe.name}</h4><span class="text-xs bg-slate-100 text-slate-500 px-2 py-0.5 rounded uppercase font-bold tracking-wide">${recipe.cat || "Geral"}</span></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><p class="text-xs font-bold text-slate-400 uppercase mb-2">Ingredientes</p><ul class="space-y-1">${(recipe.ingredients || []).map((i) => `<li class="text-sm text-slate-600 flex items-center gap-2"><i class="fa-solid fa-circle-check text-green-500 text-[10px]"></i><span>${i.q_daily || ""}${i.u || ""} <b>${i.n}</b></span></li>`).join("")}</ul></div>
                <div><p class="text-xs font-bold text-slate-400 uppercase mb-2">Preparo</p><div class="space-y-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar">${steps.length > 0 ? steps.map((s, idx) => `<div class="text-sm text-slate-600 bg-slate-50 p-2 rounded border border-slate-100"><span class="font-bold text-indigo-500 mr-1">${idx + 1}.</span> ${s}</div>`).join("") : '<div class="text-sm text-slate-400 italic">Não informado.</div>'}</div></div>
            </div>
        </div>`;
...
window.switchPreviewWeek = function (w) {
    [1, 2, 3, 4].forEach((i) => {
        ...
        if (btn) btn.className = i === w ? "flex-1 py-2 px-3 rounded-lg border text-sm font-bold whitespace-nowrap bg-indigo-600 text-white border-indigo-600 shadow-md transform scale-105 transition-all" : "flex-1 py-2 px-3 rounded-lg border text-sm font-bold whitespace-nowrap bg-white text-slate-500 border-slate-200 hover:bg-slate-50 transition-colors";
    });
    renderWeekContent(w);
};
...
window.togglePlanRecipe = function(uniqueId) {
    ...
    if (el.classList.contains('hidden')) { el.classList.remove('hidden'); btn.innerHTML = 'Ocultar <i class="fa-solid fa-chevron-up"></i>'; } 
    else { el.classList.add('hidden'); btn.innerHTML = 'Ver <i class="fa-solid fa-chevron-down"></i>'; }
}
...
function renderWeekContent(w) {
    ...
    const cats = { cafe: { label: "Café", color: "amber", icon: "fa-mug-hot" }, almoco: { label: "Almoço", color: "orange", icon: "fa-utensils" }, lanche: { label: "Lanche", color: "pink", icon: "fa-apple-whole" }, jantar: { label: "Jantar", color: "indigo", icon: "fa-moon" } };
...
function sharePost(id, userId) {
    ...
    if (navigator.share) { navigator.share({ title: "Diet & Ride", text: "Confira esta receita!", url }).catch(() => fallbackCopy(url)); } 
    else { fallbackCopy(url); }
}

function fallbackCopy(text) {
    if (navigator.clipboard && navigator.clipboard.writeText) { navigator.clipboard.writeText(text).then(() => notify("Link copiado!")).catch(() => notify("Erro ao copiar.", "error")); } 
    else { const t = document.createElement("textarea"); t.value = text; document.body.appendChild(t); t.select(); document.execCommand('copy'); document.body.removeChild(t); notify("Link copiado!"); }
}
