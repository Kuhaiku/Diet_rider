...
async function toggleBan(email, status) {
    ...
    try {
        ...
        notify("Status atualizado!"); loadUsers();
        ...
        posts.forEach(p => {
            const row = `
                <tr class="hover:bg-slate-50 border-b border-slate-50 last:border-0">
                    <td class="px-6 py-4"><p class="font-bold text-slate-700 truncate max-w-[150px]">${p.title}</p><p class="text-xs text-slate-400 truncate max-w-[150px]">${p.description}</p></td>
                    <td class="px-6 py-4 text-sm text-slate-600">${p.author_name}<br><span class="text-xs text-slate-400">${p.author_email}</span></td>
                    <td class="px-6 py-4 text-xs text-slate-500">${new Date(p.created_at).toLocaleDateString()}</td>
                    <td class="px-6 py-4 text-right">
                        <button onclick="previewReportedPost(${p.id})" class="mr-2 text-indigo-500 hover:text-indigo-700"><i class="fa-solid fa-eye"></i></button>
                        <button onclick="deletePost(${p.id}, true)" class="text-slate-400 hover:text-red-600"><i class="fa-solid fa-trash"></i></button>
                    </td>
                </tr>`;
            tbody.innerHTML += row;
        });
...
async function loadReports() {
    ...
    try {
        ...
        reports.forEach(r => {
            ...
            card.innerHTML = `
                <div class="flex justify-between items-start mb-3"><span class="bg-red-50 text-red-600 text-xs font-bold px-2 py-1 rounded border border-red-100">Denúncia #${r.id}</span><span class="text-xs text-slate-400">${new Date(r.created_at).toLocaleDateString()}</span></div>
                ...
                <div class="mt-auto"><button onclick="previewReportedPost(${r.post_id})" class="w-full py-2 text-xs font-bold bg-indigo-50 text-indigo-700 rounded hover:bg-indigo-100 transition-colors">Ver Post</button></div>`;
...
async function previewReportedPost(postId) {
    try {
        ...
        document.getElementById('raw-json').innerText = JSON.stringify(p.content_json, null, 2);
        ...
        container.innerHTML = `
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-4 flex items-center gap-3 border-b border-slate-50">
                    <div class="w-10 h-10 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-lg">${p.author_name ? p.author_name.charAt(0).toUpperCase() : '?'}</div>
                    <div><h3 class="font-bold text-slate-800 leading-tight">${p.title}</h3><p class="text-xs text-slate-500">por ${p.author_name || 'Desconhecido'} • ${new Date(p.created_at).toLocaleDateString()}</p></div>
                </div>
                <div class="px-4 py-3"><p class="text-sm text-slate-600 whitespace-pre-line">${p.description}</p></div>
                ${ytId ? `
                ...
                <div class="px-4 py-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between opacity-70 pointer-events-none grayscale">
                    <div class="flex gap-2"><button class="px-3 py-1 bg-white border rounded"><i class="fa-solid fa-thumbs-up"></i> ${p.likes_count}</button></div>
...
function switchTab(tab) {
    ...
    if (tab === 'visual') {
        btnV.className = "flex-1 py-3 text-sm font-bold text-indigo-600 border-b-2 border-indigo-600 bg-white";
        btnJ.className = "flex-1 py-3 text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors";
        contentV.classList.remove('hidden');
        contentJ.classList.add('hidden');
    } else {
        btnJ.className = "flex-1 py-3 text-sm font-bold text-indigo-600 border-b-2 border-indigo-600 bg-white";
        btnV.className = "flex-1 py-3 text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors";
...
function notify(text, type = "success") {
    Toastify({ text: text, duration: 3000, gravity: "top", position: "center", style: { background: type === "error" ? "#ef4444" : "#22c55e", borderRadius: "8px" } }).showToast();
}